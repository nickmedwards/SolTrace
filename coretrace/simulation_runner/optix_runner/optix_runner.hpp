...
public:
    ...
    virtual SolTrace::Runner::RunnerStatus
    setup_simulation(const SolTrace::Data::SimulationData* data) override;
    virtual SolTrace::Runner::RunnerStatus
    update_simulation(const SolTrace::Data::SimulationData* data) override;
    virtual SolTrace::Runner::RunnerStatus run_simulation() override;
    virtual SolTrace::Runner::RunnerStatus
    status_simulation(double* progress = nullptr) override;
    ...
    SolTrace::Runner::RunnerStatus
    get_hp_output(std::vector<float4>&        hp_vec,
    ...
    inline uint_fast64_t get_number_rays_launched() const override
    { return get_N_sun_rays(); }
    inline uint_fast64_t get_number_rays_traced() const override
    { return m_sys.get_N_hit_rays(); }
    ...
    // WARNING: The runner is forced to use this batch size regardless of
    // available GPU memory!!!! Setting a large batch size can cause device out
    // of memory errors or degraded GPU performance. Setting a small batch size
    // can cause long run times. Care is required when using this function.
    void          set_batch_size(uint_fast64_t batch_size);
    uint_fast64_t get_batch_size() const;

    /// Set the maximum ray interaction depth. Must be called before
    /// initialize(). Depth is clamped to [2, 255] with a warning if either
    /// bound is exceeded. Defaults to DEFAULT_MAX_TRACE_DEPTH.
    void    set_max_ray_depth(uint_fast64_t depth);
    uint8_t get_max_ray_depth() const { return m_sys.get_max_ray_depth(); }

    /// Returns the number of rays terminated by reaching max_depth without
    /// being absorbed. Valid after run_simulation() completes; resets to 0 at
    /// the start of each run.
    uint_fast64_t get_N_depth_exceeded_rays() const
    { return m_sys.get_N_depth_exceeded_rays(); }

    /// Enable or disable trimming of excess rays at the end of run() so that
    /// exactly the requested number of hit rays is returned.  Enabled by
    /// default.
    ...
    // } void disable_errors() { this->include_errors = false; } void
    // enable_errors() { this->include_errors = true; }
    ...
    void set_groups(const std::vector<std::set<int32_t>>& groups)
    { m_groups = groups; }
...
private:
    ...
    std::vector<SolTrace::Result::GroupResult>
         make_grouped_results(size_t num_groups) const;
    void append_grouped_results(
        const std::vector<OptixCSP::HitRecord>&     hit_records,
        int                                         level,
        std::vector<SolTrace::Result::GroupResult>& grouped_results,
        int32_t&                                    prev_group);
    ...
    SolTrace::Runner::RunnerStatus
    setup_parameters(const SolTrace::Data::SimulationData* data);
    SolTrace::Runner::RunnerStatus
    setup_sun(const SolTrace::Data::SimulationData* data);
    SolTrace::Runner::RunnerStatus
    setup_elements(const SolTrace::Data::SimulationData* data);
