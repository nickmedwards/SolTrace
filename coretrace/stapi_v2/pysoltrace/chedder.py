...
There are 3 steps to this module
locate_header      -> find the header file to parse
_parse_header_text -> use regular expressions to find typedefs/enums/etc 
                      (ctrl+f -> `re.compile` to find definitions), 
                      organize intermediate object (_ParsedHeader)
_build_namespace   -> use intermediate object to build a `type` with 
                      attributes named like those in the header file.

NOTE:
If this is not finding stuff defined in the header file. Debug by using
a regular expression testing site (regex101.com) with the current regular 
expression string and an example that works and the one that doesn't.
...
def _describe_member(name: str,
    ...
    if depth > 1: return [f"{indent}{_name}{Style.RESET_ALL}: {Fore.RED}{_type_label(value)}{Style.RESET_ALL}"]
